...
// DEBUG - poista tuotannossa
error_log("worksites_save.php called - action: " . ($_POST['action'] ?? 'none') . ", id: " . ($_POST['id'] ?? 'none'));
...
// KORJATTU:  Käytetään oikeaa funktiota sf_csrf_validate()
if (function_exists('sf_csrf_validate')) {
    error_log("CSRF token from POST: " . ($_POST['csrf_token'] ??  'MISSING'));
    error_log("CSRF token from SESSION: " . ($_SESSION['csrf_token'] ?? 'MISSING'));
    if (sf_csrf_validate() === false) {
