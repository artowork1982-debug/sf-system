...
require_once __DIR__ . '/../includes/log_app.php'; // teillä on log_app.php käytössä indexissäkin
...
// CSRF validate jos funktio löytyy
if (function_exists('csrf_validate_post')) {
    if (csrf_validate_post() === false) {
        if (sf_is_fetch()) sf_json(['ok' => false, 'error' => 'CSRF invalid'], 403);
        header("Location: {$base}/index.php?page=settings&tab=worksites&notice=error");
        exit;
    }
}

// Yhteys: käytetään config['db'] ja mysqli fallback (varma)
$db = $config['db'] ?? null;
if (!is_array($db)) {
...
try {
    if ($action === 'toggle') {
        ...
        // lokitus (käyttää teidän log_app.php:tä)
        // action-keyt kannattaa yhtenäistää audit-logiin: worksite_activated / worksite_deactivated
